...
public class CursoController {
    ...
    @PostMapping
    public ResponseEntity<CursoDto> cadastrar(@RequestBody CursoForm cursoForm, UriComponentsBuilder uriBuilder) {
        Curso curso = cursoForm.converter(cursoRepository); 
        cursoRepository.save(curso);

        URI uri = uriBuilder.path("/cursos/{id}").buildAndExpand(curso.getId()).toUri(); // Correção da criação da URI
